...
w_is=tf.Variable(1.0)
w_dc=tf.Variable(1.0)
w_fc=tf.Variable(5.0)
w_fs=tf.Variable(5.0)
w_j=tf.Variable(1.0)
...
#add square 
loss = w_ic*tf.reduce_sum(tf.square(c_ic))+w_is*tf.reduce_sum(tf.square(s_ic))+w_dc*tf.reduce_sum(tf.square(c_dcb-c_dc))+0.2*w_fc*tf.reduce_sum(tf.square(fc))+0.2*w_fs*tf.reduce_sum(tf.square(fs))+w_j*tf.reduce_sum(tf.square(j))
...
plt.plot(aa, 5*(w_fcb-w_fcb[0])/50000, label="w_fc",linewidth=4)
plt.plot(aa, 5*(w_fsb-w_fsb[0])/50000, label="w_fs",linewidth=4)
...
cbtc.to_csv(r'C:\Users\shikhar\PycharmProjects\mesh\autoencoder-for-denoising-coarser-mesh-based-numerical-solution\def\forward result\btc conventional.csv')
