...
btcpd = pd.read_csv("btc_inverse2.csv")

rppd = pd.read_csv("rp_inverse2.csv")
...
def net_NS_IC( x, y, t):
    ...
    return c_ic
  """  
def net_NS( x, t,c_out,s_out,ka,kd):
...
t_fin=tf.compat.v1.placeholder(tf.float32, shape=[None, x_ic.shape[1]])
...
w_is=tf.Variable(10.7)
w_dc=tf.Variable(3.0)
w_fc=tf.Variable(10004.0)
w_fs=tf.Variable(10005.0)

w_cm=tf.Variable(3.0)
w_sm=tf.Variable(3.0)


ka=tf.Variable(0.001)
kd=tf.Variable(0.001)
...
tfinp = np.empty((len(x_ic),1))
tfinp.fill(3600)


tf_dict = {x_dcb: x_lb, t_dcb: t_lb, x_neb: x_rb, t_neb: t_rb, x_i: x_ic, t_i: t_ic, x_f: xx_f, t_f: tt_f,t_fin:tfinp }
...
s_res=neural_net(tf.concat([x_i, t_fin], 1), weights_s,biases_s)
fc, _,_=net_NS(x_f,t_f,c_f,s_f,ka,kd)
_,fs,_=net_NS(x_f,t_f,c_f,s_f,ka,kd)
_,_,j=net_NS(x_neb,t_neb,c_neb,s_neb,ka,kd)
"""
...
loss = w_ic*tf.reduce_sum(tf.square(c_ic))+w_is*tf.reduce_sum(tf.square(s_ic))+w_dc*tf.reduce_sum(tf.square(c_dcb-c_dc))+w_sm*tf.reduce_sum(tf.square(c_neb-btc))+w_cm*tf.reduce_sum(tf.square(s_res-rp))+w_fc*tf.reduce_sum(tf.square(fc))+w_fs*tf.reduce_sum(tf.square(fs))
...
train_op_Adam = optimizer_Adam.minimize(loss,var_list=[weights_c,biases_c,weights_s,biases_s,ka,kd])  


train_op_Adam_max = optimizer_Adam_max.minimize(loss_max,var_list=[w_ic,w_dc,w_is,w_fc,w_fs,w_cm,w_sm])
...
cbtc.to_csv(r'C:\Users\shikhar\PycharmProjects\mesh\autoencoder-for-denoising-coarser-mesh-based-numerical-solution\def\forward result\btc enp inverse.csv')
