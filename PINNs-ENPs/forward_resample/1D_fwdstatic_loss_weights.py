# -*- coding: utf-8 -*-
"""
Created on Mon Sep 19 03:11:23 2022
...
w_fc=tf.Variable(100.0)
w_fs=tf.Variable(100.0)
w_j=tf.Variable(1.0)
...
#add square 
loss = w_ic*tf.reduce_sum(tf.square(c_ic))+w_is*tf.reduce_sum(tf.square(s_ic))+w_dc*tf.reduce_sum(tf.square(c_dcb-c_dc))+0.2*w_fc*w_fc*tf.reduce_sum(tf.square(fc))+0.2*w_fs*w_fs*tf.reduce_sum(tf.square(fs))+w_j*tf.reduce_sum(tf.square(j))
...
cat = pd.DataFrame(cneb)
cat.to_csv(r'C:\Users\shikhar\PycharmProjects\mesh\autoencoder-for-denoising-coarser-mesh-based-numerical-solution\def\forward result\btc_staticweight.csv')
