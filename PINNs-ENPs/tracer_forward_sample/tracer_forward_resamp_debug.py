...
for it in range(nIter):
    ...
    if it%500==0:
        ...
        newp=np.array([fp[int(i)] for i in new])           #actual points which got resampled
...
aa,gen_fin=resam(newp)   #here gen_fin is list of refined point for each oriningal point, and aa is the list of all those refined points

plt.scatter(xx_f,tt_f, marker='.')
plt.scatter(newp[:,0],newp[:,1], marker='*')
plt.scatter(aa[:,0],aa[:,1], marker='.')
plt.ylim(-1, 1000.0)
plt.xlim(-0.01, 0.205)
plt.show()

aa,gen_fin=resam(newp) 
