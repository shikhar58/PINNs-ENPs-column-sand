...
loss = w_ic*tf.reduce_sum(tf.square(c_ic))+w_dc*tf.reduce_sum(tf.square(c_dcb-c_dc))+w_cm*tf.reduce_sum(tf.square(c_neb-btc))+w_fc*tf.reduce_sum(tf.square(fc))
...
train_op_Adam = optimizer_Adam.minimize(loss,var_list=[weights_c,biases_c,por,alpha])  


train_op_Adam_max = optimizer_Adam_max.minimize(loss_max,var_list=[w_ic,w_dc,w_fc,w_cm])
