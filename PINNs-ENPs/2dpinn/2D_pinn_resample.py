...
for it in range(nIter):
    print("............")
    for i in range(0,tot,bs):
        tf_dict = {x_i: x_ic, y_i: y_ic, t_i: t_ic, x_dcb: x_dc, y_dcb: y_dc, t_dcb: t_dc,c_dct:c_dc, x_neb: x_neu, y_neb: y_neu, t_neb: t_neu, x_nebdc: x_neudc, y_nebdc: y_neudc, t_nebdc: t_neudc, x_f: newp[i:i+bs,0:1], y_f:newp[i:i+bs,1:2], t_f:newp[i:i+bs,2:3]}
        sess.run(train_op_Adam, tf_dict)
        loss_value=loss.eval(feed_dict=tf_dict,session=sess)
        #por_value=por.eval(session=sess)
        print(it,i,loss_value)
